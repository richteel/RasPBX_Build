...
if (isset($_GET['id']))
{
	...
	$current_type_qry = $db->query($type_chk_query);
	
	if ($tp = $current_type_qry->fetch_assoc())
	{
...
function carefull_delete ($tmp_id,$current_type)
{
	...
	*/
	...
	//echo $count;
	$countRES = $db->query($count);

	if ($ct = $countRES->fetch_assoc())
	{
		if ($ct['total'] > '0')
		{
...
function output_edit_object ($myId, $current_type)
{
	...
	*/
	...
	$theRES = $db->query($theSQL);

	if ($in = $theRES->fetch_assoc())
	{
...
function dropdown_menu($member_of, $indent,$member_of_container)
{
	...
	$conRESULT = $db->query($conQRY);
	
	while ($mo2 = $conRESULT->fetch_assoc())
	{
