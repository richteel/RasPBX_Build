...
if ($ph_sec == 'Yes' && $registered == 'FALSE')
{
	...
	$theobjRES = $db->query($objQuery);
	

	if ($in = $theobjRES->fetch_assoc())
	{
	...
	$theContactRES = $db->query($browseQuery);

	if ($in = $theContactRES->fetch_assoc())
	{
...
function list_contacts ($member_cat,$style,$obID,$MAC)
{
	...
	*/
	...
	$theCountRES = $db->query($countQuery);
	
	//Fetch total items
	if ($in = $theCountRES->fetch_assoc())
	{
		$totalCount = $in['total'];
	}
	...
	if ($totalCount != "0")
	{
		...
		$browseQuery = "SELECT 
			...
			$limitstart";
		$theBrowseRES = $db->query($browseQuery);
		...
		if ($style == 'Seperate')
		{
			...
			while ($in2 = $theBrowseRES->fetch_assoc())
			{
